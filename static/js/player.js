...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const submitEmotionBtn = document.getElementById("submitEmotionBtn");
    if (submitEmotionBtn) {
        submitEmotionBtn.addEventListener("click", submitEmotion);
    }
    resumeSession();
});
...
function startLegoSensePolling() {
    ...
    fetchLegoSenseView();
    state.legoSensePoll = setInterval(fetchLegoSenseView, 5000);
}
...
async function submitEmotion() {
    ...
    if (!emotion) {
        document.getElementById("emotionHint").textContent = "请先输入一个情绪词";
        return;
    }
    try {
        ...
        document.getElementById("emotionHint").textContent = "已提交，等待主持人分组。";
        fetchLegoSenseView();
    } catch (err) {
        alert(err.message);
    }
...
function renderLegoSense(view) {
    const statusEl = document.getElementById("legoSenseStatus");
    const groupEl = document.getElementById("legoSenseGroup");
    const colorEl = document.getElementById("legoSenseColor");
    const hintEl = document.getElementById("emotionHint");
    if (statusEl) {
        statusEl.textContent = legoSenseStatusLabel(view.status);
    }
    if (groupEl) {
        groupEl.textContent = view.groupName ?? "-";
    }
    if (colorEl) {
        if (view.color) {
